...
export default function Hero() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative h-svh bg-dark dot-grid-bg overflow-hidden flex flex-col justify-center"
    >
      {/* Floating corner squares */}
      {cornerPositions.map((pos, i) => (
        <div key={i} className={`absolute w-1 h-1 bg-blue/10 ${pos}`} />
      ))}
      ...
      >
        ...
        <circle cx="220" cy="180" r="80" fill="rgba(74,184,240,0.08)" />
        {/* Radial glow */}
        <defs>
      ...
      >
        {words.map((word, i) => (
          ...
          >
            <span className="hero-word inline-block font-anton text-[12vw] lg:text-[11vw] leading-[0.9] text-blue font-light">
              {word}
            </span>
          </div>
        ))}
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-6 lg:px-12 w-full mt-8 lg:mt-12">
        ...
        >
          Stop building slow. We ship your SaaS 3× faster with production-grade
          quality.
        </p>
      </div>
